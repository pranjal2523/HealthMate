...
def patview(request, consultation_id):
    ...
    if request.method == 'GET':
      ...
      return render(request,'patient/patient_ui/pat_view.html', {"puser":consultation_obj})
...
def patdrview(request, doctorusername):
   duser = User.objects.get(username=doctorusername)
   r = rating_review.objects.filter(doctor=duser.doctor)
   return render(request,'doctor/doctor_ui/patientview.html', {"duser":duser, "rate":r} )
