  ...
  if request.method == 'POST': #and request.FILES['img']:
    try:
      if request.FILES['img']:
          patimage = request.FILES['img']
          fs = FileSystemStorage(location='media/')
          filename = fs.save(patimage.name, patimage)
          url = fs.url(filename)
    except :
      puser = User.objects.get(username=patientusername)
      url = puser.patient.patimage 
      print(url)       
    name =  request.POST['name']
    dob =  request.POST['dob']
    gender =  request.POST['gender']
    ...
    #patimage = request.FILES.get('img')
    #print(dob)
    dobdate = datetime.strptime(dob,'%Y-%m-%d')
    puser = User.objects.get(username=patientusername)
    #print(patimage)
    patient.objects.filter(pk=puser.patient).update(name=name,dob=dobdate,gender=gender,address=address,mobile_no=mobile_no, patimage=url)
...
def saveddata(request,doctorusername):
  ...
  if request.method == 'POST': #and request.FILES['img']:
    try:
      if request.FILES['img']:
          docimage = request.FILES['img']
          fs = FileSystemStorage(location='media/')
          filename = fs.save(docimage.name, docimage)
          url = fs.url(filename)
    except :
      duser = User.objects.get(username=doctorusername)
      url = duser.doctor.docimage 
      print(url)   
    name =  request.POST['name']
    dob =  request.POST['dob']
    gender =  request.POST['gender']
    ...
    doctor.objects.filter(pk=duser.doctor).update( name=name, dob=dob, gender=gender, address=address, mobile_no=mobile_no, registration_no=registration_no, year_of_registration=yor, qualification=qualification, State_Medical_Council=State_Medical_Council, specialization=specialization, docimage=url )
